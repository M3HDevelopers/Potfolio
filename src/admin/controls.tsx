...
export function OtpModal({
  ...
  const submit = async () => {
    ...
    /* No demoCode and no remote check: just collect the code (backend forgot flow). */
    if (!demoCode) {
      onVerified(trimmed);
      return;
    }
    if (trimmed === demoCode) onVerified(trimmed);
    else setError("Incorrect code. Check the demo banner above and try again.");
  };
